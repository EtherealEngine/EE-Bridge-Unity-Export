...
Shader "Skybox/Panoramic (Rotate2)" {
Properties {
    _Tint ("Tint Color", Color) = (.5, .5, .5, .5)
    [Gamma] _Exposure ("Exposure", Range(0, 8)) = 1.0
    _Rotation ("Rotation", Range(0, 360)) = 0
	_Rotation2 ("Vertical Rotation", Range(0, 360)) = 0
...
SubShader {
    ...
    Pass {
        ...
        inline float2 ToRadialCoords(float3 coords)
        {
            ...
            //float latitude = acos((normalizedCoords.y + _Rotation2) % 1.0);
		...
		float3 RotateAroundXInDegrees(float3 vertex, float degrees)
		{
			...
			sincos(alpha, sina, cosa);
			float2x2 m = float2x2(cosa, -sina, sina, cosa);
			return float3(vertex.x, mul(m, vertex.yz)).xyz;
		}
        ...
        v2f vert (appdata_t v)
        {
			...
			rotated = RotateAroundXInDegrees(rotated, _Rotation2);
